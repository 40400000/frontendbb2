        ...
        <div className="grid grid-cols-1 md:grid-cols-4 min-h-[60vh] md:items-start">
          ...
          <div className="md:col-span-1"> {/* Removed flex flex-col h-full */}
            ...
            <div className="space-y-4 pt-8 md:pt-8 lg:pt-16 md:pl-0"> {/* Re-added md:pt-8 lg:pt-16 */}
              {topics.map((topic) => (
                <Link key={topic} href={topicLinks[topic] || '#'} passHref>
                  <div
                    onMouseEnter={() => handleMouseEnter(topic)}
                    className={`flex items-center text-5xl lg:text-6xl font-semibold cursor-pointer transition-colors duration-200 ${
                      selectedTopic === topic
                  ...
                  >
                    {topic}
                    {selectedTopic === topic && (
                      <MdOutlineArrowOutward className="ml-4 h-10 w-10 transition-opacity duration-300 opacity-100" />
                    )}
                  </div>
                </Link>
              ))}
