...
export function AnimatedHeroBackground({
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      mouseSmooth[0] += (mouse.x - mouseSmooth[0]) * 0.025;
      mouseSmooth[1] += (mouse.y - mouseSmooth[1]) * 0.025;

      const resolution = [gl.canvas.width, gl.canvas.height];
      gl.uniform2fv(u_resolution, resolution);
      gl.uniform1f(u_time, (Date.now() - startTime) * 0.0025);
      gl.uniform2fv(u_mouse, mouseSmooth);
      gl.uniform1f(u_dpr, window.devicePixelRatio);
      
      const col1Array = color1.split(",").map(Number);
      const col2Array = color2.split(",").map(Number);
      const col3Array = color3.split(",").map(Number);
      
      gl.uniform3fv(u_col1, col1Array);
      gl.uniform3fv(u_col2, col2Array);
      gl.uniform3fv(u_col3, col3Array);
